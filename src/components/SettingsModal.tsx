...
import { X, Printer, Monitor, Settings as SettingsIcon, Server, Smartphone, Info, PlayCircle, Cloud, Save, Database, Loader2, Plus, Zap } from 'lucide-react';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  // ✅ Detect if wrapped in the Native Android App
  const isNativeBridge = typeof window !== 'undefined' && !!window.AndroidPrinter;
  ...
  const handleSunmiOneClick = () => {
    ...
    setConnectedPrinter({
        ...
        name: isNativeBridge ? 'Sunmi Native App' : 'Sunmi Internal Printer',
        type: 'network',
        status: 'connected',
        ipAddress: '127.0.0.1',
        port: '9100'
  ...
  const handleTestPrint = async () => {
    ...
    Method: ${isNativeBridge ? 'Native App Bridge' : 'Localhost / RAWBT'}
          ...
          {activeTab === 'printer' && (
            <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-neutral-900 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-neutral-800">
                    ...
                    {!connectedPrinter && (
                        <div className={`border rounded-xl p-4 mb-6 ${isNativeBridge ? 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-200 dark:border-indigo-800' : 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800'}`}>
                            <div className="flex gap-4 items-start">
                                <div className={`p-2.5 rounded-xl mt-1 shrink-0 ${isNativeBridge ? 'bg-indigo-100 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-300' : 'bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300'}`}>
                                    {isNativeBridge ? <Zap size={24} /> : <Smartphone size={24} />}
                                </div>
                                <div className="flex-1">
                                    <h4 className="font-bold text-slate-900 dark:text-white text-lg leading-tight mb-1">
                                      {isNativeBridge ? 'Sunmi Native App (Detected)' : 'RawBT / Sunmi Setup'}
                                    </h4>
                                    <p className="text-sm text-slate-600 dark:text-neutral-300 mb-3">
                                      {isNativeBridge 
                                        ? 'App is wrapped! Printing will bypass RawBT and use the hardware directly.' 
                                        : 'One-click setup for Sunmi devices or Android RawBT.'}
                                    </p>
                                    <button onClick={handleSunmiOneClick} className={`w-full sm:w-auto text-white text-sm font-bold py-2.5 px-5 rounded-lg flex items-center justify-center gap-2 transition-all ${isNativeBridge ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-blue-600 hover:bg-blue-700'}`}>
                                        <div className="w-2 h-2 bg-white rounded-full animate-ping" />
                                        {isNativeBridge ? 'Connect Native Printer' : 'One-Click Connect (Localhost)'}
                                    </button>
                                </div>
                            </div>
                        </div>
                    ...
                    {connectedPrinter ? (
                        <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-lg p-4 flex justify-between items-center mb-6">
                            <div className="flex items-center gap-3">
                                ...
                                <p className="text-xs text-emerald-600 dark:text-emerald-400 uppercase font-bold flex items-center gap-1">
                                  {isNativeBridge ? 'NATIVE ANDROID SDK' : `${connectedPrinter.type} ${connectedPrinter.ipAddress}`}
                                </p>
                                </div>
                            </div>
