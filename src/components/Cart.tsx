...
export const Cart: React.FC<CartProps> = ({ 
  ...
  const subtotal = items.reduce((sum, item) => {
    ...
    if (item.manualPrice !== undefined) {
      ...
      itemTotal = (item.price + item.modifiers.reduce((acc, mod) => acc + mod.price, 0)) * item.quantity;
    }
              ...
              <div className="flex justify-between items-start mb-2">
                ...
                <span className="font-mono font-medium text-slate-600">
                  £{(item.manualPrice !== undefined 
                      ? item.manualPrice * item.quantity 
                      : (item.price + item.modifiers.reduce((acc, mod) => acc + mod.price, 0)) * item.quantity
                    ).toFixed(2)}
                </span>
              </div>
